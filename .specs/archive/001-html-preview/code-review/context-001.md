...
async function previewAllHtml(): Promise<void> {
  ...
  for (const header of headers) {
    ...
    const html = await fetchPreviewHtml(rawUrl);
    toggleInlinePreview(container, html);
  }
}
...
it('opens preview page via window.open with extension URL', async () => {
  ...
  expect(window.open).toHaveBeenCalledWith(
    'chrome-extension://mock-id/preview.html?id=mock-uuid',
    '_blank'
  );
});
...
export function addPreviewButtonToHeader(header: Element, rawUrl: string): void {
  ...
  const panelBtn = createPreviewButton('Panel', () => {
    fetchPreviewHtml(rawUrl).then(html => showInPanel(html, fileName));
  });
  insertPreviewButton(header, panelBtn);

  // Inline button
  const inlineBtn = createPreviewButton('Inline', () => {
    if (diffContainer) {
      fetchPreviewHtml(rawUrl).then(html => toggleInlinePreview(diffContainer, html));
    }
  });
